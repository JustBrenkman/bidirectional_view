...
/// This is only because using `onScaleUpdate` and `onPanUpdate`
/// throws an error with a normal `GestureDetector` and I want
/// to have both independently since `onScaleUpdate` does not
/// store any delta information.
...
class _ScalingGestureDetectorState extends State<ScalingGestureDetector> {
  ...
  void _onTouchUpdate(Touch touch, DragUpdateDetails details) {
    ...
    if (_touches.length == 1) {
      ...
      touch._startOffset = details.localPosition;
    } else {
      ...
      _initialScalingDistance = newDistance;
    }
  }
